...
module.exports = async function handler(req, res) {
  ...
  const { from, to, erro } = validarPeriodo(params);
  ...
  try {
    ...
    // Qualificado = 10+ colaboradores, a mesma regra que dispara o
    // LeadQualificado no Meta (lib/porte.js).
    const qualificados = countBy(leadsData.filter(l => ehQualificado(l.colaboradores)), 'pagina');

    return res.status(200).json(montarResposta({ visits, leads, qualificados, visitantesPor }));
...
/** from/to (ou since/until) validados, ou `erro` pronto para virar 400. */
function validarPeriodo(params) {
  const from = params.get('from') || params.get('since'); // ex: "2026-04-01"
  const to   = params.get('to')   || params.get('until'); // ex: "2026-04-09"
  const invalido = (from && !DATE_RE.test(from)) || (to && !DATE_RE.test(to)) || (from && to && from > to);
  return { from, to, erro: invalido ? 'from/to (ou since/until) devem estar em YYYY-MM-DD, com início <= fim' : null };
}
...
/** Comparação por página (sem lista fixa: página nova aparece sozinha) + o
 * relatório antigo do A/B (`variants`, sempre as 3 páginas históricas). */
function montarResposta({ visits, leads, qualificados, visitantesPor }) {
