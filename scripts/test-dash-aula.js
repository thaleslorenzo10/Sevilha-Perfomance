...
ok(/D\.nomeChave/.test(aula), 'junção campanha × meta usa D.nomeChave');
ok(aula.split('\n').length <= 350, 'aula.js abaixo de 350 linhas');
const tabelas = fs.readFileSync(path.join(RAIZ, 'assets/js/dash/tabelas.js'), 'utf8');
ok(/custo_compra:/.test(tabelas), 'COLS de tabelas.js tem custo_compra');
console.log(falhas ? `\n${falhas} falha(s)` : '\nTudo certo');
