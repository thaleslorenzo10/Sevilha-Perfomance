...
/**
 * Total de leads de uma linha de insights, na métrica que corresponde à
 * mecânica da campanha.
 *
 * `lead` é a métrica agregada do Meta: soma formulário instantâneo (onsite) +
 * lead do pixel no site (offsite). Fora das campanhas de formulário isso é o
 * número certo, e é o que continua valendo aqui.
 *
 * Dentro de `[FORMS]`, não: ali o lead É o formulário preenchido, e o pixel
 * entra por cima contando gente que viu o anúncio e converteu por outro
 * caminho. No acumulado de 21/12/2025 a 11/08/2026 a soma dava 2.944 contra
 * 1.586 formulários — 1,9× — e mostrava o lead a R$ 16,50 quando custava
 * R$ 30,63. O onsite bate com a planilha de leads reais (1.564, 98,6%), que é
 * a checagem que sustenta esta escolha.
 *
 * A classificação sai do nome da campanha, então linhas sem `campaign_name`
 * (nível de conta) continuam no agregado em vez de zerar em silêncio.
 */
function extractLeads(row) {
  if (row.campaign_name && classifyCampaign(row.campaign_name).formato === 'FORMS') {
    return extractOnsiteLeads(row);
  }
 ...
 *   [AULA]  → Aula paga (/aula-gestao-operacional)
 *   [CAFÉ]  → Café com Sevilha (landing page do RD Marketing)
