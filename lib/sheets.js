...
async function getAccessToken() {
  ...
  try {
    ...
    const pistas = [];
    if (!sa.key.includes('BEGIN PRIVATE KEY')) {
      pistas.push('falta a linha "-----BEGIN PRIVATE KEY-----" (cole o campo private_key inteiro do JSON)');
    }
    if (!sa.key.includes('\n')) {
      pistas.push('a chave veio numa linha só — as quebras de linha (\\n) se perderam');
    }
    throw new Error(
      'GOOGLE_PRIVATE_KEY não é uma chave RSA válida'
      + (pistas.length ? `: ${pistas.join('; ')}` : ` (${e.message})`)
    );
  }
...
async function readViaServiceAccount() {
  ...
  // Se os gids das abas de FORMS foram informados, lê só elas — menos dados
  // trafegados e imune a outras abas de trabalho da planilha.
  const gidsAlvo = gidsDeForms();
  if (gidsAlvo.length) {
    const filtradas = abas.filter(a => gidsAlvo.includes(a.gid));
    const ausentes  = gidsAlvo.filter(g => !abas.some(a => a.gid === g));
    ...
    if (ausentes.length) {
      console.warn(`[sheets] gids configurados e não encontrados na planilha: ${ausentes.join(', ')}`);
    }
    if (filtradas.length) abas = filtradas;
    else console.warn('[sheets] nenhum gid configurado bateu; lendo todas as abas');
  } else {
    // Sem gid configurado, acha as abas do export pelo título ("Leads Forms",
    // "Leads Forms 2", ...). É o único caminho que não quebra sozinho quando
    // alguém cria a próxima aba — por isso a Service Account é preferível ao
    // CSV por gid, que exige lembrar de atualizar a variável.
    const doExport = abas.filter(a => /^leads\s*forms/i.test(String(a.titulo).trim()));
    if (doExport.length) abas = doExport;
  }
