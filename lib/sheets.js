...
// A gravação (lib/sheets-write.js) pede escopo de escrita; o token é o mesmo
// mecanismo, só muda o escopo pedido — por isso o cache é por escopo.
...
const cachedTokens = new Map(); // escopo → { token, exp }
...
async function getAccessToken(scope = SCOPE) {
  ...
  cachedTokens.set(scope, { token: json.access_token, exp: now + (json.expires_in || 3600) });
...
async function abasViaServiceAccount(sheetId, { gids = [], tituloRe = null, rotulo = 'planilha' } = {}) {
  ...
  return abas.map((a, i) => ({
    ...a,
    rows: valJson.valueRanges?.[i]?.values || [],
...
module.exports = { readAllTabs, readLPTabs, parseCsv, normalizarChave, getAccessToken, getServiceAccount, SHEETS_API, SCOPE_RW };
