...
async function readViaCsv() {
  ...
  const out = [];
  const erros = [];
  // A URL tentada vai junto no erro: sem ela, um LEADS_SHEET_ID com a URL
  // inteira colada dentro (engano comum) é indistinguível de um problema de
  // permissão — os dois dão 401. O id de uma planilha não é segredo: quem o
  // tem continua sem acesso se ela for privada.
  const tentativas = [];
  ...
  for (const url of urls) {
    ...
    try {
      ...
      // Planilha sem acesso público se manifesta de duas formas: 401/403, ou
      // 200 devolvendo a página de login em HTML. Sem estas checagens o CSV
      // "vazio" viraria zero lead em silêncio, o pior desfecho possível.
      ...
      if (semAcesso) {
        erros.push(
          `aba ${gid}: o Google exigiu login (HTTP ${res.status}) — a planilha não está ` +
          'acessível sem autenticação. Deixe-a como "qualquer pessoa com o link pode ver", ' +
          'ou publique a aba em CSV e use LEADS_SHEET_CSV_URLS'
        );
        continue;
      }
      if (!res.ok) {
