 ...
 */
function montarUserData({ email, telefone, nome, pais, cidade, estado, cep, externalId, leadId, fbp, fbc, fbclid, ip, userAgent, quandoMs }) {
  const ud = {};
  ...
  if (cidade)   ud.ct = [hash(String(cidade).trim().toLowerCase().replace(/[^\p{L}\p{N}]/gu, ''))];
  if (estado)   ud.st = [hash(String(estado).trim().toLowerCase().replace(/[^\p{L}\p{N}]/gu, ''))];
  if (cep)      ud.zp = [hash(String(cep).replace(/\D/g, '').slice(0, 5))];
  if (nome) {
  ...
  // Aceita mais de um: o id do navegador e o e-mail são external_id diferentes
  // para a mesma pessoa, e é o CRM que usa o e-mail. Mandar os dois é o que faz
  // o Lead da landing page e o evento do CRM caírem sobre a mesma pessoa.
  const externos = [...new Set((Array.isArray(externalId) ? externalId : [externalId])
    .map(v => String(v || '').trim().toLowerCase())
    .filter(Boolean))];
  if (externos.length) ud.external_id = externos.map(hash);
 ...
 */
async function enviarEvento({ evento, eventId, quando, userData, customData, sourceUrl, referrerUrl, actionSource }) {
  ...
  if (referrerUrl) dado.referrer_url     = referrerUrl;
  if (customData) dado.custom_data = customData;

  try {
    // A aba "Testar eventos" do Gerenciador só mostra evento que carrega este
    // código. Sem ele o evento chega normalmente e aparece na visão geral, mas
    // quem está validando não vê nada e conclui que não funcionou. Deixar a
    // variável definida em produção é o oposto do que se quer: o evento vira
    // teste e não conta como conversão — por isso ela só existe quando alguém
    // está de fato validando.
    const corpo = { data: [dado] };
    const codigoDeTeste = process.env.META_TEST_EVENT_CODE;
    if (codigoDeTeste) {
      corpo.test_event_code = codigoDeTeste;
      console.warn(`[capi] META_TEST_EVENT_CODE ativo (${codigoDeTeste}) — ${evento} vai como TESTE e não conta como conversão`);
    }
