...
async function responder(req, res) {
  ...
  try {
    const [leadsRes, viewsRes, funil] = await Promise.all([
    ...
    const porFonte = {};
    const porCampanha = {};
    const porPagina = Object.fromEntries(PAGINAS.map(p => [p, { leads: 0, visitas: 0, conversao: null }]));
    ...
    for (const p of Object.values(porPagina)) {
      p.conversao = p.visitas ? (p.leads / p.visitas) * 100 : null;
    }
