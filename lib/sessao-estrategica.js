...
async function responder(req, res) {
   ...
   *
   * `criado_em` é o nome confirmado em produção em 03/09/2026 e vem primeiro
   * para a primeira tentativa acertar. Os outros ficam como rede: o dia em que
   * a tabela for recriada com outro nome, o relatório se conserta sozinho em
   * vez de mostrar zero visitante como se ninguém tivesse entrado.
   */
  const COLUNAS_DATA = ['criado_em', 'created_at', 'inserted_at', 'timestamp', 'data'];
  ...
  try {
     ...
     *
     * A contagem de visitante começou em 03/09/2026, com o evento `pageview`.
     * Período que comece antes disso tem lead de dias que não têm visitante
     * registrado, e a divisão passa de 100% — foi o que apareceu no primeiro
     * deploy: 5 leads para 4 visitantes, "125% de conversão".
     *
     * Mais lead do que visitante é impossível e denuncia o denominador
     * incompleto sem precisar guardar a data de corte em lugar nenhum. Nesse
     * caso o número volta para a base antiga, e `base_conversao` diz qual foi
     * usada — para ninguém comparar duas taxas que não medem a mesma coisa.
     */
    function base(leads, visitantes, visitas) {
      if (visitantes && leads <= visitantes) return { valor: visitantes, nome: 'visitantes' };
      if (visitas) return { valor: visitas, nome: 'visitas' };
      return { valor: 0, nome: null };
    }

    for (const p of Object.values(porPagina)) {
      const b = base(p.leads, p.visitantes, p.visitas);
      p.conversao = b.valor ? (p.leads / b.valor) * 100 : null;
      p.base_conversao = b.nome;
    }

    const baseTotal = base(leads.length, totalVisitantes, views.length);
    ...
    return res.status(200).json({
      ...
      conversao: baseTotal.valor ? (leads.length / baseTotal.valor) * 100 : null,
      base_conversao: baseTotal.nome,
      ...
      nota_visitas: !views.length
        ...
        : baseTotal.nome === 'visitantes'
          ? null
          : !visitantes.coluna
            ? `Não foi possível contar visitantes distintos: nenhuma coluna de data conhecida existe em ${TABELAS.eventosPagina}. A conversão está por carregamento, não por pessoa.`
            : totalVisitantes
              ? 'Período começa antes de 03/09/2026, quando a contagem de visitante distinto entrou no ar: há lead de dias sem visitante registrado. A conversão está por carregamento até o período ficar inteiro dentro da medição nova.'
