...
async function responder(req, res) {
  ...
  function url(tabela, select) {
    ...
    if (since) u += `&created_at=gte.${since}T00:00:00${FUSO}`;
    if (until) u += `&created_at=lte.${until}T23:59:59${FUSO}`;
    return u;
  }
  ...
  try {
    const [leadsRes, viewsRes, visitantesRes, funil] = await Promise.all([
      ...
      // 185 carregamentos vindos de anúncio no Meta — a taxa de conversão saía
      // pela metade do que era. O evento `pageview` carrega o id do visitante.
      fetch(`${url(TABELAS.eventosPagina, 'pagina,visitante')}&evento=eq.pageview`, { headers }),
      buscarFunil(),
    ]);
    ...
    const marcas = visitantesRes.ok ? await visitantesRes.json() : [];
