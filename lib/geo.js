...
// Host do serviço web GeoLite2: é o que a chave gratuita abre. O serviço pago
// (GeoIP2 Precision) fica em geoip.maxmind.com, com o MESMO caminho e o mesmo
// formato de resposta — trocar de um para o outro é trocar esta linha. Chave
// GeoLite2 batendo no host pago devolve PERMISSION_REQUIRED, que é um erro fácil
// de ler como "credencial errada" quando na verdade é produto errado.
//
// Limite: 1.000 consultas por dia por conta. Acima disso a MaxMind recusa, o
// código devolve null e o evento sai sem localização — nunca quebra o lead.
...
/**
 * Devolve `{ cidade, estado, cep, pais, raioKm, asn }` ou `null`.
 *
 * `asn` é o nome da operadora dona do IP. Não vai para a Meta: existe para
 * responder depois a única pergunta que importa aqui — quanto desta
 * localização vem de gateway de operadora móvel, que é onde o palpite erra.
 *
...
async function localizarPorIp(ip) {
  ...
  try {
    ...
    resultado = {
      ...
      raioKm,
      asn:    json?.traits?.autonomous_system_organization || null,
    };
