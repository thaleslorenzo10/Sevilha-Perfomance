...
/** Compras da aula pagas no período (pago_em, no fuso do painel): a compra
 * pode acontecer dias depois do lead, fora do recorte de leads. */
async function lerCompras(since, until) {
  ...
  if (!url) return { modo: null, registros: [] };
  return { modo: 'postgrest', registros: await lerTudo(url) };
}
...
async function montarUnificados(since, until) {
  const [forms, respondi, supabase, compras] = await Promise.all([
    tolerante(lerForms()), tolerante(lerRespondi()), tolerante(lerSupabase(since, until)),
    tolerante(lerCompras(since, until)),
  ]);
  ...
  if (compras.erro) console.error('[leads-unificados] compras da aula indisponíveis:', compras.erro);
  const payload = unificar({
    forms: forms.registros, respondi: respondi.registros, supabase: supabase.registros, compras: compras.registros,
  }, since, until);
