...
// `ini` já chega aqui avançado pelo tamanho do lote recebido (nunca por PAG
// fixo), porque o max-rows do servidor pode entregar menos que PAG por
// página. Com o total do Content-Range sabemos exatamente quando parar; sem
// o header (ex.: stubs de teste), volta ao critério antigo.
function acabou({ ini, lote, total, PAG }) {
  return total !== null ? (ini >= total) : (lote.length < PAG || lote.length === 0);
}
...
async function lerTudo(url, headersExtra = {}) {
  ...
  const linhas = [];
  let truncado = true;
  for (let pagina = 0, ini = 0; pagina < MAX_PAGINAS; pagina++) {
    ...
    linhas.push(...lote);
    ini += lote.length; // avança pelo que veio de fato, não por PAG fixo
...
module.exports = { TABELAS, conexao, rest, lerTudo, totalDoContentRange };
