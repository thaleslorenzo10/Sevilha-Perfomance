...
const { rest, lerTudo } = require('./supabase');
...
// `leads` fica na interface (o brief e os chamadores passam por ela) mas não é
// usada aqui — cruzamento com sessão vem pelos `deals` do RD, não da lista de leads.
function agregarCompras({ compras, leads: _leads, deals, since, until }) {
...
/** Compras da tabela sevilha_compras_aula no período (folga de 1 dia no fuso do painel). */
async function lerCompras(since, until) {
  const url = rest('sevilha_compras_aula',
    `pago_em=gte.${since}T00:00:00-03:00&pago_em=lte.${until}T23:59:59-03:00&order=pago_em.asc`);
  if (!url) return [];
  return lerTudo(url);
}

async function unificar({ forms = [], respondi = [], supabase = [], deals = [] }, since, until) {
  const todos = [
  ...
  // Compras só quando fazem sentido: página da aula com lead no período, ou
  // já houve compra antes (não perder venda cujo lead saiu do recorte).
  let compras = null;
  if (porGrupo.AULA.leads > 0) {
    const brutas = await lerCompras(since, until);
    if (brutas.length) compras = agregarCompras({ compras: brutas, leads: noPeriodo, deals, since, until });
  }
