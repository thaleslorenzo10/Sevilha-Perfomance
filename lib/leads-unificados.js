...
async function unificar({ forms = [], respondi = [], supabase = [], deals = [] }, since, until) {
  ...
  // Busca por período (pago_em), não por lead no período: a compra pode
  // acontecer dias depois do lead ter sido criado, fora do recorte de leads.
  const brutas = await lerCompras(since, until);
  const compras = brutas.length ? agregarCompras({ compras: brutas, leads: noPeriodo, deals, since, until }) : null;
