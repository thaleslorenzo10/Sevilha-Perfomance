...
const URL_DASHBOARD = process.env.DASHBOARD_URL || 'https://sevilha-perfomance.vercel.app/dashboard';
...
function nomeCurto(nome) {
  return String(nome || '')
    .replace(/\[(CP|SE|AULA|FORMS|LEAD|FASE\d+)\]\s*/g, '')
...
async function montarResumo(p) {
  ...
  const captacao = meta.grupos.SE.spend + meta.grupos.CP.spend + (meta.grupos.AULA ? meta.grupos.AULA.spend : 0);
  ...
  return {
    ...
    campanhas,
    anuncios: await melhoresAnuncios(since, until, temLeads ? leads : null),
 ...
 */
async function melhoresAnuncios(since, until, leadsPlanilha, quantos = 5) {
  const { fetchAdInsights, fetchAdInstagramLinks, classifyCampaign } = require('./meta');

  try {
    const rows = await fetchAdInsights(since, until);

    /**
     * O mesmo criativo é duplicado em várias campanhas e conjuntos, e cada
     * cópia é um ad_id diferente. Sem consolidar, o ranking mostra o mesmo
     * anúncio três vezes com número parcial em cada linha.
     *
     * A chave é o NOME do anúncio: cópias com o mesmo nome viram uma linha
     * só, somando investimento e leads. O público entra na linha — quando as
     * cópias rodaram em conjuntos diferentes, isso fica visível em vez de
     * sumir na soma.
     */
    ...
    for (const r of rows) {
      ...
      if (!g) {
        g = { nome, ids: [], publicos: new Set(), formatos: new Set(), spend: 0, link: null };
        grupos.set(nome, g);
      }

      g.ids.push({ id: r.ad_id, spend });
      if (r.adset_name) g.publicos.add(String(r.adset_name).trim());
      g.formatos.add(classifyCampaign(r.campaign_name).formato);
      g.spend += spend;
    }

    /**
     * O ranking usa os LEADS DA PLANILHA, não os do Meta.
     *
     * O Meta reporta a conversão modelada: no AD29, 21 leads em três dias
     * contra 6 preenchimentos de formulário de verdade. Usar esse número aqui
     * colocaria dois significados de "lead" na mesma imagem — o card diz
     * "Leads reais" e a lista diria outra coisa, 3x maior.
     *
     * O nome do anúncio existe nos dois lados: o export do Meta traz ad_name,
     * e a landing page grava {{ad.name}} no utm_content.
     */
    const reaisPorAnuncio = new Map(
      (leadsPlanilha?.por_anuncio || []).map(a => [a.anuncio, a])
    );

    const anuncios = [...reaisPorAnuncio.values()]
      .filter(a => a.leads > 0)
      .map(a => {
        const g = grupos.get(a.anuncio);
        const spend = g ? Math.round(g.spend * 100) / 100 : null;
        return {
          nome: a.anuncio,
          // Ordena as cópias por investimento: o link sai da mais relevante.
          ids: g ? g.ids.sort((x, y) => y.spend - x.spend).map(x => x.id) : [],
          copias: g ? g.ids.length : 0,
          publico: !g || g.publicos.size === 0 ? ''
                 : g.publicos.size === 1 ? [...g.publicos][0]
                 : `${g.publicos.size} públicos`,
          formato: a.FORMS > 0 && a.LP > 0 ? 'MISTO' : a.LP > 0 ? 'LP' : 'FORMS',
          spend,
          leads: a.leads,
    ...
    // Busca o link de todas as cópias e fica com o primeiro que existir,
    // começando pela de maior investimento.
    const todosIds = anuncios.flatMap(a => a.ids);
    const links = await fetchAdInstagramLinks(todosIds);
    for (const a of anuncios) {
      a.link = a.ids.map(id => links.get(id)).find(Boolean) || null;
    }
...
function montarTexto(r) {
  ...
  if (r.anuncios?.length) {
    L.push('');
    L.push('🏆 *Melhores anúncios* (por leads reais)');
    for (const a of r.anuncios) {
      const tag = a.formato === 'MISTO' ? 'FORM+LP' : a.formato === 'FORMS' ? 'FORM' : 'LP';
      L.push(`   • [${tag}] ${nomeCurto(a.nome)}`);
      // Um nome pode existir na planilha sem entrega no Meta no período (lead
      // que chegou depois, anúncio pausado). Aí não há custo a mostrar, e
      // "R$ 0,00/lead" seria pior que a ausência do número.
      L.push(a.cpl !== null
        ? `     ${int(a.leads)} leads · ${brl(a.cpl)}/lead · ${brl0(a.spend)}`
        : `     ${int(a.leads)} leads · sem investimento no período`);
      const detalhe = [a.publico, a.copias > 1 ? `${a.copias} cópias somadas` : null].filter(Boolean).join(' · ');
  ...
  L.push('');
  L.push(`📊 Dashboard completo: ${URL_DASHBOARD}`);

  return L.join('\n');
}

module.exports = { montarResumo, montarTexto, URL_DASHBOARD, periodoDe, brl, brl0, int, pct, dia, nomeCurto };
