...
async function melhoresAnuncios(since, until, quantos = 5) {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      if (!g) {
        g = {
          nome, ids: [], publicos: new Set(), formatos: new Set(),
          spend: 0, leads: 0, impressions: 0, clicks: 0, link: null,
        };
        grupos.set(nome, g);
      }
      ...
      g.spend       += spend;
      g.leads       += qtd;
      g.impressions += parseFloat(r.impressions || 0);
      g.clicks      += parseFloat(r.clicks || 0);
    }
    ...
    const anuncios = [...grupos.values()]
      ...
      .map(g => ({
        ...
        ids: g.ids.sort((a, b) => b.spend - a.spend).map(x => x.id),
        copias: g.ids.length,
        publico: g.publicos.size === 1 ? [...g.publicos][0]
               : g.publicos.size > 1  ? `${g.publicos.size} públicos`
               : '',
        formato: g.formatos.size === 1 ? [...g.formatos][0] : 'MISTO',
        spend: Math.round(g.spend * 100) / 100,
        leads: g.leads,
        cpl: Math.round((g.spend / g.leads) * 100) / 100,
        ctr: g.impressions > 0 ? Math.round((g.clicks / g.impressions) * 1000) / 10 : null,
        link: null,
      }))
