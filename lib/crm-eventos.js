 ...
 */
async function montarEvento(deal, evento) {
...
async function varrerCrm() {
  ...
  for (let page = 1; page <= PAGINAS_MAX; page++) {
    ...
    for (const bruto of deals) {
      ...
      if (quando && quando < limite) { truncou = true; continue; }
      const evento = eventoDe(deal);
  ...
  for (const { deal, evento, eventId, quando } of novos) {
    ...
    const resultado = await enviarEvento({
      ...
      ...(await montarEvento(deal, evento)),
      actionSource: 'system_generated',
    });
  ...
  const resumo = { ok: true, paginas, candidatos: candidatos.length, ja_enviados: jaForam.size, enviados, falhas, sem_contato: semContato, truncou };
  console.log('[CRM varredura]', JSON.stringify(resumo));
  return resumo;
}

module.exports = { tratarWebhook, varrerCrm, eventoDe, lerDeal, contatosDoDeal, PADROES, EVENTO_GANHO };
