...
async function leadOriginal(email) {
  ...
  try {
    const res = await fetch(
      ...
      `?select=telefone,fbclid,fbp,fbc,external_id,page_url,user_agent,event_id,pagina,utm_campaign,utm_source,utm_content,cargo,escritorio,colaboradores,geo_cidade,geo_estado,geo_cep,created_at` +
...
async function montarEvento(deal, evento, indiceLeadId) {
  ...
  const userData = montarUserData({
    ...
    nome:       deal.contato,
    pais:       'br',
    // O e-mail e o id que o navegador gerou na visita original: é o que faz o
    // evento do CRM cair sobre a mesma pessoa do Lead da landing page.
    externalId: [deal.email, original?.external_id].filter(Boolean),
    leadId:     leadId || undefined,
    // Cookies e navegador da visita original. O CRM não os tem, o Supabase sim,
    // e são os parâmetros de maior peso na correspondência depois do e-mail.
    fbp:        original?.fbp || undefined,
    fbc:        original?.fbc || undefined,
    fbclid:     original?.fbclid,
    userAgent:  original?.user_agent || undefined,
    // Aqui a localização por IP é o único lugar onde ela acrescenta alguma
    // coisa: este evento sai sem IP, então a Meta não tem como deduzir sozinha
    // o que deduziria num evento de navegador.
    cidade:     original?.geo_cidade || undefined,
    estado:     original?.geo_estado || undefined,
    cep:        original?.geo_cep    || undefined,
  ...
  const customData = {
    ...
    utm_source:    original?.utm_source  || undefined,
    utm_content:   original?.utm_content || undefined,
    cargo:         original?.cargo       || undefined,
    escritorio:    original?.escritorio  || undefined,
  ...
  // A página onde a pessoa se inscreveu. O evento nasce no CRM, mas a origem
  // real é a landing page — e o Meta usa a URL como sinal de atribuição.
  return { userData, customData, sourceUrl: original?.page_url || undefined };
}
...
async function tratarWebhook(req, res) {
  ...
  const { userData, customData, sourceUrl } = await montarEvento(deal, evento);
